...
EVENT['LOG_MSGID_LIST'] = ['LOG-1001', 'LOG-1002']
...
class EventHandler():
    ...
    def action_one(self):
        """The first action that the script should do on execution."""
        self.logger.info('Thing One')
        print('Thing One')
        return

    def action_two(self):
        """The second action that the script should do on execution."""
        self.logger.info('Thing Two')
        print('Thing Two')
        return

    def do_actions(self):
        """Executes the defined actions.
        This can be customized based on the log stored in
        self.data['log_messages'] depending on the need.
        """
        self.__get_logs()
        # this loop can be replaced with other code that calls the actions
        # directly, or conditionally based on the message.

...
def main():
    ...
    cli_output = exsh.clicmd('debug vsm show peer | inc "vrId      :"', True).split()
    isc_vlan = cli_output[5]
    peer_name = exsh.clicmd('debug vsm show peer | include "Name      : "', True).split()
    ...
    if cli_output[5] == 'Unknown':
        ...
        print  # Check the number of ports added to the ISC vlan, and get the port number of the ISC link

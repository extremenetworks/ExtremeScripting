...
/**
 * This is an example of using paging with to get all VLANs on a switch.
 ...
 * 
 */
public class GetWithPagingExample {

    /**
     * Fetch all VLANs. This sends a request to get all VLANs from the switch
     * using paging.
     * 
     * @param stub
     * @param session 
     */
    public void fetchAllVLANs(SwitchPortType stub, Session session) {
	Utilities.log("fetchAllVLANs:");
	...
	try {
		...
		for(int i=0;i<numberOfObjectsReturned;i++) {
		    VlanConfig vlanConfig=(VlanConfig)response.getObjects()[i];
		    Utilities.log("VLAN: "+vlanConfig.getName()+" Tag: "+vlanConfig.getTagValue());
		}

		//Next batch is retrieved by sending index of last row
		//The index for a VlanConfig is the name of the VLAN
		VlanConfig lastVlan=(VlanConfig)response.getObjects()[numberOfObjectsReturned-1];
		filter.setName(lastVlan.getName());
	...
	Utilities.log("fetchAllVLANs: DONE");
    }


    /**
     * Main execution method
     * Usage: GetWithPagingExample switch=<switch> username=<username> password=<password>
     * 
	...
	if(device==null || username==null) {
	    System.out.println("Usage: GetWithPagingExample switch=<switch> username=<username> password=<password>");
	    System.exit(1);
	}
	...
	try {
	    ...
	    //Execute operations
	    GetWithPagingExample getWithPagingExample=new GetWithPagingExample();
	    getWithPagingExample.fetchAllVLANs(stub, session);
